//! `SignedField` — a field element carrying an explicit sign.
//!
//! This type was `noirc_frontend::signed_field::SignedField` up to Noir
//! `v1.0.0-beta.19`. Upstream deleted it in
//! <https://github.com/noir-lang/noir/pull/11710> (`v1.0.0-beta.20`) when
//! `ast::Literal::Integer` moved from `SignedField` to a bare `FieldElement`
//! whose sign is recovered from the accompanying `ast::Type`.
//!
//! Verno's loop-unrolling constant interpreter (`vir_backend::lowering::loop_unroll`)
//! is built on this type: it needs signed arithmetic over field-sized values that
//! is *independent* of any particular integer width, which neither `FieldElement`
//! (unsigned, wraps mod p) nor `i128` (too narrow for `Field`) provides. Rather
//! than re-model that interpreter, the type is vendored here verbatim from
//! Noir `v1.0.0-beta.13` (the revision Verno was previously pinned to), minus the
//! `rangemap::StepLite` impl and the serde derives, which Verno never used.
//!
//! Conversion to and from the `beta.26` representation lives in
//! `vir_backend::lowering::loop_unroll::{signed_field_from_literal, ...}`.
//!
//! Upstream source: MIT/Apache-2.0, noir-lang/noir.

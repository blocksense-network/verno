        ...
        match expr {
            ExprF::Literal { value } => {
                let (exprx, span_msg) = match value {
                    crate::annotations::ast::Literal::Bool(val) => {
                        (ExprX::Const(Constant::Bool(val)), format!("Const boolean {}", val))
                    }
                    crate::annotations::ast::Literal::Int(big_int) => (
            ...
            ExprF::UnaryOp { op, expr } => {
                let (exprx, span_msg) = match op {
                    crate::annotations::ast::UnaryOp::Not => {
                    ...
                    crate::annotations::ast::UnaryOp::Dereference => {
